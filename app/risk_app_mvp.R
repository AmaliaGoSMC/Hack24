...
# Ensure main_risk_cat is in lowercase
#data_cleaned$main_risk_cat <- tolower(data_cleaned$main_risk_cat)
    ...
    mainPanel(
      tabsetPanel(
        tabPanel("Summary", 
                 ...
                 fluidRow(
                   column(4, uiOutput("card_total_risks")),
                   column(4, uiOutput("card_total_mitigations")),
                   column(4, uiOutput("card_total_cost"))
                 ),
                 
                 br(), br(),
                 
                 fluidRow(
                   column(4, uiOutput("card_avg_cost_pre")),
                   column(4, uiOutput("card_avg_cost_post")),
...
server <- function(input, output, session) {
  ...
  output$card_total_cost <- renderUI({
    stats <- summary_stats()
    div(style = card_style, h4("Total Risk Cost (£)"), h3(format(stats$total_cost, big.mark = ",")))
  })
  
  output$card_avg_cost_pre <- renderUI({
    stats <- summary_stats()
    div(style = card_style, h4("Avg Cost Pre-Mitigation (£)"), h3(format(stats$avg_cost_pre, big.mark = ",")))
  })
  
  output$card_avg_cost_post <- renderUI({
    stats <- summary_stats()
    div(style = card_style, h4("Avg Cost Post-Mitigation (£)"), h3(format(stats$avg_cost_post, big.mark = ",")))
  })
  
  output$card_avg_probability <- renderUI({
    stats <- summary_stats()
    div(style = card_style, h4("Avg Probability (Pre & Post)"),
        h3(paste(round(stats$avg_prob_pre * 100, 1), "% /", round(stats$avg_prob_post * 100, 1), "%")))
  })
